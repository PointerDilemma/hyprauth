...
static void pamConversationV1_method0(Hyprwire::IObject* r) {
    auto& fn = rc<CPamConversationV1Object*>(r->getData())->m_listeners.client_ready;
    if (fn)
        fn();
}

static void pamConversationV1_method1(Hyprwire::IObject* r, const char* token_bytes) {
    auto& fn = rc<CPamConversationV1Object*>(r->getData())->m_listeners.success;
    if (fn)
        fn(token_bytes);
}

static void pamConversationV1_method2(Hyprwire::IObject* r, const char* token_bytes, const char* message) {
    auto& fn = rc<CPamConversationV1Object*>(r->getData())->m_listeners.fail;
    if (fn)
        fn(token_bytes, message);
}

static void pamConversationV1_method3(Hyprwire::IObject* r, const char* message) {
    auto& fn = rc<CPamConversationV1Object*>(r->getData())->m_listeners.pam_prompt;
    if (fn)
        fn(message);
}

static void pamConversationV1_method4(Hyprwire::IObject* r, const char* message) {
    auto& fn = rc<CPamConversationV1Object*>(r->getData())->m_listeners.pam_text_info;
    if (fn)
        fn(message);
}

static void pamConversationV1_method5(Hyprwire::IObject* r, const char* message) {
...
CPamConversationV1Object::CPamConversationV1Object(Hyprutils::Memory::CSharedPointer<Hyprwire::IObject>&& object) : m_object(std::move(object)) {
    ...
    m_object->listen(5, rc<void*>(::pamConversationV1_method5));
}
...
void CPamConversationV1Object::sendFinished() {
    m_object->call(1);
}

void CPamConversationV1Object::sendPamResponseChannel(int messageFd) {
    m_object->call(2, messageFd);
}

void CPamConversationV1Object::setClientReady(std::function<void()>&& fn) {
    m_listeners.client_ready = std::move(fn);
}
...
std::vector<SP<Hyprwire::SServerObjectImplementation>> CHyprauthPamV1Impl::implementation() {
    return {
        ...
        makeShared<Hyprwire::SServerObjectImplementation>(Hyprwire::SServerObjectImplementation{.objectName = "pam_conversation_v1",
                                                                                                .version    = m_version,
                                                                                                .onBind =
                                                                                                    [this](Hyprutils::Memory::CSharedPointer<Hyprwire::IObject> r) {
                                                                                                        if (m_bindFn)
                                                                                                            m_bindFn(r);
                                                                                                    }}),
    };
